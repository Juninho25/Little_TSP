...
class Solution:
    ...
    def update_cost_matrix(self,
                           cordinates):  # dostaje krotke wsp., nic nie zwraca; modyfikuje(zabrania powrotnego przejscia) self.cost_matrix
    ...
    def get_path(self):  # zwraca listę kolejnych wierzchołków w sciezce korzystając z self.cost_matrix 2x2 i self.path
        unsorted = []
        pth = []
        ...
        for i in range(s_uns):
            for j in range(s_uns):
                if unsorted[j].coordinates[0] == col:
                    ...
                    pth.append(unsorted[j].coordinates[0])
                    break
        self.path = pth
        return pth
...
def main():
    matrix = np.array(
        [[inf, 10, 8, 19, 12], [10, inf, 20, 6, 3], [8, 20, inf, 4, 2], [19, 6, 4, inf, 7], [12, 3, 2, 7, inf]])
